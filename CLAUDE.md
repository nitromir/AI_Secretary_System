...
**Wiki RAG & Knowledge Base**: `app/services/wiki_rag_service.py` — BM25 Okapi retrieval with Russian/English stemming (`snowballstemmer`) over `wiki-pages/*.md`. Indexes sections by `##`/`###` headers on startup, injects relevant context into LLM system prompt. BM25 parameters: k1=1.5, b=0.75, MIN_SCORE=0.5. Title boost 4x. Zero GPU. Initialized in `orchestrator.py` startup, stored in `ServiceContainer.wiki_rag_service`. `app/routers/wiki_rag.py` exposes admin API: stats, reload, search, and Knowledge Base document CRUD (upload/edit/delete `.md`/`.txt` files). Documents tracked in `knowledge_documents` table (`KnowledgeDocument` model), managed via `AsyncKnowledgeDocManager` in `db/integration.py`. Existing `wiki-pages/*.md` auto-synced to DB on first request. Admin UI: Finetune → LLM → Cloud AI toggle (wiki stats, knowledge base table, test search). Migration: `scripts/migrate_knowledge_base.py`.
