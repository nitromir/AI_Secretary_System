...
<template>
  <div class="relative">
    <button
      type="button"
      class="flex items-center gap-2 rounded-lg bg-zinc-800 px-3 py-2 text-sm text-gray-300 hover:bg-zinc-700 hover:text-white transition-colors"
      @click="themeStore.toggleTheme"
      :title="`Theme: ${labels[themeStore.theme]}`"
    >
      <svg
        class="h-5 w-5"
        fill="none"
      ...
      <span class="hidden sm:inline">{{ labels[themeStore.theme] }}</span>
    </button>
  </div>
</template>
