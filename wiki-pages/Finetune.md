...
#### Логика генерации Q&A пар

Датасет создаётся **без нейросети** — чистый pattern matching по коду и документации:

**Из Python-кода:**
- `@router.get/post` + docstring → *«Что делает endpoint GET /admin/...?»*
- SQLAlchemy модели → *«Какие поля есть в таблице X?»* + список полей
- `SECRETARY_PERSONAS`, `PROVIDER_TYPES` и другие dict-конфиги → *«Какие персоны/провайдеры доступны?»*
- Pydantic BaseModel → *«Какие параметры принимает X?»* + поля с типами

**Из Markdown-документации (wiki-pages/, README.md):**
- Секции `##`/`###` → вопрос по ключевым словам заголовка:
  - `install/setup` → *«Как настроить...»*
  - `api/endpoint` → *«Как использовать API...»*
  - `error/problem` → *«Как решить...»*
- Код-блоки (\`\`\`bash/python) → вопрос по типу команды:
  - `docker` → *«Как запустить через Docker?»*
  - `curl` → *«Как вызвать API?»*
  - `pytest` → *«Как запустить тесты?»*
- Таблицы → *«Какие параметры доступны?»* + таблица целиком

**Сценарии продаж** и **эскалации** — захардкожены в `finetune_manager.py` (~15 реалистичных диалогов с возражениями, ценами, кейсами).

**FAQ** — прямое копирование из БД: вопрос → ответ, без модификации.

**GitHub** — shallow clone (`--depth 1`), парсинг `*.py` + `*.md` теми же методами, удаление после обработки. Таймаут: 60 сек.
