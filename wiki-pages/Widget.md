...
Виджет автоматически сохраняет историю чата при переходе между страницами сайта (аналог Replain):

- **Cookie** (`SameSite=None; Secure`) — основное хранилище session ID, 30-дневный TTL
- **localStorage** — резервное хранилище (если cookie недоступны)
- **sessionStorage** — запоминает состояние окна (открыто/закрыто)

При загрузке каждой страницы виджет автоматически подгружает историю сообщений с сервера. Если чат был открыт — он откроется снова с полной историей.

Очистка сессии: `window.aiChat.clearSession()` (удаляет cookie + localStorage + sessionStorage).
